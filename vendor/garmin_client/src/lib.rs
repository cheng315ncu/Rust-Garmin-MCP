...
/// Non-interactive MFA. Public so callers can validate a configured secret
...
impl GarminClient {
    ...
    /// Obtain the MFA code without assuming a terminal.
    ///
    /// Upstream prompts on stdin, which is fine from a shell but impossible
    /// under MCP: stdin there carries the JSON-RPC transport, so the prompt
    /// consumes protocol bytes and login fails with an empty code. The sources
    /// below are tried in order, and the interactive path is kept for anyone
    /// running this from a real terminal.
    async fn resolve_mfa_code() -> Option<String> {
        // 1. An explicit code, for a single attended run.
        if let Ok(code) = std::env::var("GARMIN_MFA_CODE") {
            let code = code.trim().to_string();
            if !code.is_empty() {
                info!("Using MFA code from GARMIN_MFA_CODE");
                return Some(code);
            }
        }

        // 2. A TOTP secret — the only fully unattended option.
        if let Ok(secret) = std::env::var("GARMIN_TOTP_SECRET") {
            if !secret.trim().is_empty() {
                // A code that rolls over mid-request gets rejected, so wait out
                // the tail of the window rather than gambling on it.
                let remaining = totp::seconds_remaining();
                if remaining < 3 {
                    debug!("TOTP window closes in {remaining}s; waiting for the next one");
                    tokio::time::sleep(std::time::Duration::from_secs(remaining + 1)).await;
                }
                match totp::current_code(&secret) {
                    Ok(code) => {
                        info!("Generated MFA code from GARMIN_TOTP_SECRET");
                        return Some(code);
                    }
                    Err(e) => {
                        error!("{e}");
        ...
        // 3. Ask — but only where there is somebody to answer.
        if stdin().is_terminal() {
            ...
            error!("No MFA code entered");
            return None;
        }

        error!(
            "Garmin is asking for an MFA code, but stdin is not a terminal (under \
             MCP it carries the protocol). Set GARMIN_TOTP_SECRET to your \
             two-factor secret for unattended login, or GARMIN_MFA_CODE for a \
             single run."
        );
        None
    }
    ...
    async fn handle_mfa(&mut self) {
        ...
        let Some(mfa_code) = Self::resolve_mfa_code().await else {
            // Leaving the SSO response untouched makes login() fail downstream,
            // which is the same outcome as submitting a wrong code.
            return;
        };
